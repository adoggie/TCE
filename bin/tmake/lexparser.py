...
arch = '32'
lang_kws=[]
codecls = None  #代码实现语言类  codecls = Java
...
class TypeBase:
	...
	def _getTypeName(self,call_module):
...
class Sequence(Contained,TypeBase):
	def __init__(self,name,type):
		...
		syntax:
			struct Part{ .. }
			sequence< Part> body_parts_t;
			sequence< string > strlist_t;

		sequence<type> name
		"""
		TypeBase.__init__(self,name)
		self.type = type # sequence<type> name;  type 指泛型参数类型
		self.valuetype = type

	def getTypeDefaultValue(self,call_module):
		return codecls.Sequence.defaultValue(self,call_module)
	...
	def getMappingTypeName(self,call_module):
		return codecls.Sequence.typeName(self,call_module)
...
class Dictionary(Contained,TypeBase):
	...
	syntax:
		struct Part{ .. }
		dictionary< string, Part> body_parts_t;
		dictionary< string, string> strstr_t;
		dictionary<k,v>  k must be primitive type
	"""
	...
	def getTypeDefaultValue(self,call_module):
		return codecls.Dictionary.defaultValue(self,call_module)
...
# 			return '[NSMutableDictionary new]'
	...
	def getMappingTypeName(self,call_module):
		...
		return codecls.Dictionary.typeName(self,call_module)
...
class Struct(Container,Contained,TypeBase):
	...
	def getTypeName(self,call_module):
		...
		return codecls.Struct.typeName(self,'')
