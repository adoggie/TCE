...
#	sw.writeln("//# -- INTERFACE PROXY -- ")
	...
	for opidx,m in enumerate(e.list): # function list
		...
		if m.type.name =='void':
			...
			sw.define_var('r','boolean','false')
			sw.define_var('m','RpcMessage','new RpcMessage(RpcMessage.CALL|RpcMessage.ONEWAY)')
...
#			sw.writeln(';')
		...
		if True:
			...
			sw.define_var('r','boolean','false')
			sw.define_var('m','RpcMessage','new RpcMessage(RpcMessage.CALL|RpcMessage.ASYNC)')
			sw.writeln("m.ifidx		= %s;"%ifidx)
			sw.writeln("m.opidx		= %s;"%opidx)
			argc = len(m.params)
			...
			sw.writeln("r = this.conn.sendMessage(m);")
			sw.writeln("if(!r){").idt_inc()
			sw.writeln('throw "RpcConsts.RPCERROR_SENDFAILED";')
			sw.scope_end() # end if()
			sw.scope_end2() # end function()
			#end rpc proxy::call()  --
#			sw.writeln(';')
...
#		sw.scope_end() # end class  '}'
	...
	sw.writeln('prx.conn = new RpcConnection(uri);')
...
#	sw.scope_end().wln() # end construct function
	...
	for m in e.list: # function list
		...
		s = string.join( list,',')
		if s: s += ','
		sw.writeln('//public %s %s(%sRpcContext ctx){'%(m.type.getMappingTypeName(module),m.name,s ) )
...
#		sw.writeln('r = false;')
		...
		sw.writeln('var ctx = new RpcContext();')
